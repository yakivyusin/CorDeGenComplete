...
public class NaiveParallelGeneratorTests
{
    ...
    [Property(StartSize = 1296, EndSize = 3000)]
    public Property NaiveParallelGeneratorTest_PlusBasquePresenter(PositiveInt termCount)
    {
        var expectedTextBags = new CorpusGenerator(termCount.Get, ITermPresenter.Plus.SouthernEurope.Basque).GetCorpus()
            .Select(x => x.Split([" ", Environment.NewLine], StringSplitOptions.RemoveEmptyEntries).OrderBy(x => x));

        var actualTextBags = new NaiveParallelCorpusGenerator(termCount.Get, ITermPresenter.Plus.SouthernEurope.Basque, Environment.ProcessorCount).GetCorpus()
            .Select(x => x.Split([" ", Environment.NewLine], StringSplitOptions.RemoveEmptyEntries).OrderBy(x => x));
    ...
    [Property(StartSize = 1296, EndSize = 3000)]
    public Property NaiveParallelGeneratorTest_PlusCatalanPresenter(PositiveInt termCount)
    {
        var expectedTextBags = new CorpusGenerator(termCount.Get, ITermPresenter.Plus.SouthernEurope.Catalan).GetCorpus()
            .Select(x => x.Split([" ", Environment.NewLine], StringSplitOptions.RemoveEmptyEntries).OrderBy(x => x));

        var actualTextBags = new NaiveParallelCorpusGenerator(termCount.Get, ITermPresenter.Plus.SouthernEurope.Catalan, Environment.ProcessorCount).GetCorpus()
            .Select(x => x.Split([" ", Environment.NewLine], StringSplitOptions.RemoveEmptyEntries).OrderBy(x => x));
    ...
    [Property(StartSize = 1296, EndSize = 3000)]
    public Property NaiveParallelGeneratorTest_PlusGalicianPresenter(PositiveInt termCount)
    {
        var expectedTextBags = new CorpusGenerator(termCount.Get, ITermPresenter.Plus.SouthernEurope.Galician).GetCorpus()
            .Select(x => x.Split([" ", Environment.NewLine], StringSplitOptions.RemoveEmptyEntries).OrderBy(x => x));

        var actualTextBags = new NaiveParallelCorpusGenerator(termCount.Get, ITermPresenter.Plus.SouthernEurope.Galician, Environment.ProcessorCount).GetCorpus()
            .Select(x => x.Split([" ", Environment.NewLine], StringSplitOptions.RemoveEmptyEntries).OrderBy(x => x));
